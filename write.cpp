...
double PulseDuration = 0.0; // im ms
		...
		if(pModule->START_WRITE())
		{
				...
				for(;;)
				{
						...
						for(i = 0x0; i < DataStep; i++)
						{
								...
								if (CurrentTime <= PulseDuration)
								{
										WriteBuffer[i+BaseIndex] = Round((2047.0 + SignalAmplitude + fi.DacOffsetCoef[DacNumber])*fi.DacScaleCoef[DacNumber]);
								}
								if((CurrentTime > PulseDuration) && (CurrentTime <= 2*PulseDuration))
								{
										WriteBuffer[i+BaseIndex] = Round((2047.0 + fi.DacOffsetCoef[DacNumber])*fi.DacScaleCoef[DacNumber]);
								}	
								...
								//WriteBuffer[i + BaseIndex] |= (WORD)(0x1 << 15) | (WORD)(0x1 << 14);
						}
						RequestNumber ^= 0x1;
